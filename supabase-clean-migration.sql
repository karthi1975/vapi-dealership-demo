-- Clean Migration Script for VAPI Squads Database
-- This script drops all existing tables and creates everything fresh
...
DROP TABLE IF EXISTS trade_in_evaluations CASCADE;
...
DROP FUNCTION IF EXISTS get_call_analytics(DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_customer_journey(VARCHAR) CASCADE;
...
-- 2. Create calls table (depends on customers)
CREATE TABLE calls (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- 3. Create all tables that depend on calls
...
-- 4. Create tables that depend on both customers and calls
...
CREATE TABLE trade_in_evaluations (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    call_id UUID REFERENCES calls(id),
    customer_id UUID REFERENCES customers(id),
    vehicle_make VARCHAR(50),
...
CREATE INDEX idx_customer_intents_call_id ON customer_intents(call_id);
CREATE INDEX idx_vehicle_interests_call_id ON vehicle_interests(call_id);
CREATE INDEX idx_test_drive_bookings_customer_id ON test_drive_bookings(customer_id);
CREATE INDEX idx_financing_applications_customer_id ON financing_applications(customer_id);
CREATE INDEX idx_trade_in_evaluations_customer_id ON trade_in_evaluations(customer_id);
CREATE INDEX idx_agent_performance_agent_date ON agent_performance(agent_name, date);
