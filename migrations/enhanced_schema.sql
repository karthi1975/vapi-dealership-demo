-- Enhanced Schema for VAPI Dealership System
...
CREATE TABLE IF NOT EXISTS call_transcripts (
    ...
    customer_id UUID REFERENCES customers(id),
    transcript JSONB NOT NULL, -- Store as JSON with timestamps
    summary TEXT,
...
CREATE INDEX idx_inventory_search ON inventory(year, make, model, price, is_available);
CREATE INDEX idx_vehicle_interests_call ON vehicle_interests(call_id);
CREATE INDEX idx_transcripts_call ON call_transcripts(call_id);
CREATE INDEX idx_communications_customer ON communication_logs(customer_id, type);
CREATE INDEX idx_shared_links_code ON shared_links(short_code);